<template>
    <div>
        <div class="tools-bar mb-3 text-end">
            <router-link :to="{name: 'addGenieSkill'}" class="btn btn-primary">Создать скилл джина</router-link>
        </div>
        <div class="responsive-table overflow-auto">
            <table class="table text-center table-borderless align-middle">
                ...
                <tbody>
                    <tr v-for="(item, index) in skills" :class="{ 'table-active': hoveredTrId == index }" :key="index" @mouseover="hoveredTrId = index" @mouseout="hoveredTrId = -1" @click="$router.push(`/updateGenieSkill/id=${item.id}`)">
                        <td>{{item.id}}</td>
                        <td>{{item.title}}</td>
                        <td>{{item.descr}}</td>
                        <td>
