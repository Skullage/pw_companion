		...
		<div class="row">
				...
				<div class="grid">
					<TransitionGroup name="list">
						<calc-form
							...
							:data="form.data"
							:whitelist="whitelist"
							:miniResultVisible="forms.length > 1"
						/>
			...
			>
				<div class="offcanvas-header">
					<h4 class="sidebar__title mb-0" id="offcanvasScrollingLabel">Дом</h4>
					<button
						type="button"
						class="btn-close"
				...
				<div class="offcanvas-body">
					<Transition name="title" mode="out-in">
						...
						<h5 class="sidebar__title text-center" v-else>Необходимо</h5>
					</Transition>
...
export default {
	data() {
		return {
			...
			forms:
				localStorage.getItem('forms') != undefined
	...
	methods: {
		...
		calculate(event) {
			this.forms[
				this.forms.findIndex(item => item.id == event.id)
			].resources[0].value = event.resources[0].value
			this.forms[
				this.forms.findIndex(item => item.id == event.id)
			].resources[1].value = event.resources[1].value
			this.forms[
				this.forms.findIndex(item => item.id == event.id)
			].resources[2].value = event.resources[2].value
			this.forms[
				this.forms.findIndex(item => item.id == event.id)
			].resources[3].value = event.resources[3].value
			this.forms[
				this.forms.findIndex(item => item.id == event.id)
			].resources[4].value = event.resources[4].value
			this.forms[
				this.forms.findIndex(item => item.id == event.id)
			].resources[5].value = event.resources[5].value
			this.forms[
				this.forms.findIndex(item => item.id == event.id)
			].resources[6].value = event.resources[6].value
			this.sum()
		},
		sum() {
			this.reset()
			this.forms.forEach(el => {
				...
				this.resources[6].value < el.resources[6].value
					? (this.resources[6].value = el.resources[6].value)
					: this.resources[6].value
			})
		},
		addForm() {
			this.forms.push({
		...
		increaseValue(event) {
			this.forms[this.forms.findIndex(item => item.id == event.id)].data[
				event.param
			] += 1
			localStorage.setItem('forms', JSON.stringify(this.forms))
		},
